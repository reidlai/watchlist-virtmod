...
describe("WatchlistRxService (In-Memory)", () => {
    ...
    beforeEach(() => {
        vi.useFakeTimers();
        service = new WatchlistRxService({ apiBaseUrl: "http://localhost:8000", usingMockData: true });
    });
    ...
    describe("Initialization", () => {
        it("should initialize with empty state", async () => {
            const tickers = await firstValueFrom(service.watchlist$.pipe(map(w => w.tickers)));
    ...
    describe("fetchTickers", () => {
        it("should fetch tickers successfully", async () => {
            ...
            const promise = firstValueFrom(service.watchlist$.pipe(
                map(w => w.tickers),
                filter(t => t.length > 0)
            ));
